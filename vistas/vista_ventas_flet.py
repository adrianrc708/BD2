...
# Estado temporal del producto que se está eligiendo antes de agregarlo al carrito
...
def vista_ventas(page: ft.Page):
    ...
    # Dropdown que mostrará: "Nombre - $Precio (Stock: X)"
    ...
    # Control de Cantidad con Flechitas (+ / -)
    ...
    def buscar_producto(e):
        ...
        try:
            cur = conn.cursor()
            # Buscamos ID, Nombre, Precio, Stock
            sql = "SELECT id_producto, nombre, precio, stock FROM productos WHERE LOWER(nombre) LIKE LOWER(:1)"
            cur.execute(sql, [f"%{termino}%"])
            rows = cur.fetchall()

            dd_productos.visible = True
            # Formato opción: "Coca Cola - $15.00 (Disp: 100)"
            opts = []
            for r in rows:
                texto_display = f"{r[1]} - ${r[2]} (Disp: {r[3]})"
                opts.append(ft.dropdown.Option(text=texto_display, key=str(r[0])))
    ...
    def cargar_producto_seleccionado(id_prod):
        # Consultamos BD para tener datos frescos de precio y stock
        conn = db_connector.get_connection()
        if not conn: return
        try:
            cur = conn.cursor()
    ...
    def agregar_producto_al_carrito(e):
        if not producto_actual["id"]:
            mostrar_snack("Primero busque y seleccione un producto", "orange")
            return

        cant = int(txt_cant.value)

        # Validar Stock
        if producto_actual["stock"] < cant:
            mostrar_snack(f"Stock insuficiente. Disponible: {producto_actual['stock']}", "red")
            return
        ...
        # Resetear campos de producto para la siguiente busqueda
    ...
    def actualizar_tabla():
        ...
        for i, it in enumerate(carrito_productos):
            ...
            tabla_carrito.rows.append(ft.DataRow(cells=[
                ft.DataCell(ft.Text(it["nombre"])),
                ft.DataCell(ft.Text(f"${it['precio']:.2f}")),
    ...
    def finalizar_venta(e):
        ...
        try:
            ...
            cur.execute(
                ...
                [cliente_seleccionado["id"], DEFAULT_EMPLEADO_ID, 0, id_v])
            ...
            dlg = ft.AlertDialog(title=ft.Text("Venta Exitosa"), content=ft.Text(f"Venta #{int(v_id)} registrada."))
            page.open(dlg)
    ...
    def guardar_nuevo_cliente(e):
        if not txt_new_nombre.value or not txt_new_dni.value:
            mostrar_snack("Datos incompletos", "red")
            return
        conn = db_connector.get_connection()
        try:
            cur = conn.cursor()
    ...
    dialog_nuevo_cliente = ft.AlertDialog(
        title=ft.Text("Nuevo Cliente"),
        content=ft.Column([txt_new_nombre, txt_new_apellido, txt_new_dni], height=200, width=300),
        actions=[
            ft.TextButton("Cancelar", on_click=lambda e: page.close(dialog_nuevo_cliente)),
    ...
    col_controles = ft.Column([
        ft.Text("DATOS DEL CLIENTE", color=ft.Colors.GREY_500, weight="bold", size=12),
        ft.Row(
            [txt_buscar_cliente, ft.IconButton(ft.Icons.SEARCH, on_click=buscar_cliente, icon_color=ft.Colors.INDIGO)],
            spacing=5),
        dd_clientes,  # VISIBLE SOLO SI HAY RESULTADOS

        ft.Row([
            ft.ElevatedButton("Nuevo", on_click=lambda e: page.open(dialog_nuevo_cliente),
                              style=ft.ButtonStyle(bgcolor=ft.Colors.TEAL_500, color="white"), expand=True),
            ft.ElevatedButton("Público Gral.", on_click=seleccionar_generico,
        ...
        ft.Text("AGREGAR PRODUCTO", color=ft.Colors.GREY_500, weight="bold", size=12),
        ft.Row([txt_buscar_producto,
                ft.IconButton(ft.Icons.SEARCH, on_click=buscar_producto, icon_color=ft.Colors.INDIGO)], spacing=5),
        dd_productos,  # VISIBLE SOLO SI HAY RESULTADOS

        ft.Row([
            ft.Text("Cantidad:", weight="bold"),
            btn_menos, txt_cant, btn_mas
        ], alignment=ft.MainAxisAlignment.CENTER),

        ft.ElevatedButton("AGREGAR AL CARRITO", icon=ft.Icons.ADD_SHOPPING_CART,
    ...
    col_tabla = ft.Column([
        ft.Text("CARRITO DE COMPRAS", color=ft.Colors.GREY_800, weight="bold", size=16),
        ft.Container(tabla_carrito, border=ft.border.all(1, ft.Colors.GREY_200), border_radius=8, padding=5,
                     expand=True),
        ft.Divider(),
        ft.Row([
            ft.Column([ft.Text("TOTAL A PAGAR", size=12, color="grey"), lbl_total]),
